...
export class FormInput extends React.PureComponent<Props> {
  ...
  render() {
    ...
    const {id, label, placeholder, name, type, field, colNum, disabled, className, hideValid, suffix, step} = this.props;
    return (
      <Form.Group as={Col} lg={colNum} controlId={name} className={className}>
        ...
        <InputGroup>
          ...
          />
          {!!this.props.suffix && <InputGroup.Text>{suffix}</InputGroup.Text>}
