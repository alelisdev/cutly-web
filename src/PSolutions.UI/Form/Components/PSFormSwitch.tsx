...
export class PsFormSwitch extends React.PureComponent<Props> {
  ...
  render() {
    ...
    return (
      <Form.Group as={Col} controlId={name} className={className}>
        <FormCheck id={id} name={name} label={label} type="checkbox" checked={field.value} className="form-switch" onChange={this.handleChange}/>
      </Form.Group>
    );
  }
}